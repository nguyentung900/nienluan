		...
		<div class="container">
			...
			<?php 
				...
				foreach ($dataArr_catename as $value_catename) {
					...
					if($value_catename['cate_name']==$cate_name) {
						$sql_catename_toload="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and cate_name='$cate_name'";
					}
				}
				$query_cate_toload=mysqli_query($conn, $sql_catename_toload);
				$dataArr_catename_toload=array();
				while($row=mysqli_fetch_assoc($query_cate_toload)){
					$dataArr_catename_toload[]=$row;
				}
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_catename_toload)) { ?>
					<?php foreach($dataArr_catename_toload as $key_loadpro => $value_loadpro): ?>
