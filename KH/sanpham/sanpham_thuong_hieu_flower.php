	...
	<div class="content">
		...
		<?php if(isset($dataArr_proname)){include_once 'search_proinfo.php';} else { ?>
			...
			<?php 
				$cate_name;
				$khoatrang;
				$pro_flower=trim(substr($khoatrang, 21));
				//echo $cate_name.' '.$khoatrang;
				...
				foreach ($dataArr_catename as $value_catename) {
					...
					if($value_catename['cate_name']==$cate_name) {
						foreach($dataArr_flower as $value_proflower_toload){
							
							if($value_proflower_toload==$pro_flower){
								$sql_catename_toload="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and cate_name='$cate_name' and pro_flower='$pro_flower'";
							}
				...
				$query_flower_toload=mysqli_query($conn, $sql_catename_toload);
				$dataArr_proflower_toload=array();
				while($row=mysqli_fetch_assoc($query_flower_toload)){
					$dataArr_proflower_toload[]=$row;
				}
					/*echo '<pre>';
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_proflower_toload)) { ?>
					<?php foreach($dataArr_proflower_toload as $key_loadpro => $value_loadpro): ?>
