	...
	<div class="content">
		<div class="container">
				...
				<div class="w3ls_dresses_grid_left_grid">
					...
					<div class="w3ls_dresses_grid_left_grid_sub">
						<div class="ecommerce_color">
							<ul>
								<li><a href="main.php?khoatrang=sanpham_thuong_hieu011&ma=<?php echo $gia ?>"><i></i>Có hoa</a></li>
			...
			<?php 
				$cate_name;
				$khoatrang;
				$pro_color=trim(substr($khoatrang, 21));
				//$cate_name.' '.$khoatrang;
				...
				foreach ($dataArr_catename as $value_catename) {
					...
					if($value_catename['cate_name']==$cate_name) {
						foreach($dataArr_pro_color as $value_procolor_toload){
							$pro_color_toload=$value_procolor_toload['pro_color'];
							if($value_procolor_toload['pro_color']==$pro_color){
								$sql_catename_toload="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and cate_name='$cate_name' and pro_color='$pro_color_toload'";
							}
				...
				$query_color_toload=mysqli_query($conn, $sql_catename_toload);
				$dataArr_procolor_toload=array();
				while($row=mysqli_fetch_assoc($query_color_toload)){
					$dataArr_procolor_toload[]=$row;
				}
					/*echo '<pre>';
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_procolor_toload)) { ?>
					<?php foreach($dataArr_procolor_toload as $key_loadpro => $value_loadpro): ?>
