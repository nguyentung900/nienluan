	...
	<div class="content">
		<div class="container">
			...
			<?php 
				$gia;
				$khoatrang;
				$color=trim(substr($khoatrang, 13));
				...
				foreach($dataArr_price as $key_price => $value_price){
					...
					if($gia==$value_price) {
						//echo $gia1.' '.$gia2;
						foreach($dataArr_pro_color as $key_loadpro => $value_loadpro) {
							if($color==$value_loadpro['pro_color']){
								$sql_pro_color_toload="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and pro_color='$color' and pro_newprice between $gia1 and $gia2";
							}
						}
					}
				}
				$query_pro_color_toload=mysqli_query($conn, $sql_pro_color_toload);
				$dataArr_pro_color_toload=array();
				while($row=mysqli_fetch_assoc($query_pro_color_toload)){
					$dataArr_pro_color_toload[]=$row;
				}
				/*echo '<pre>';
				print_r($dataArr_pro_color_toload);
				echo '</pre>';
*/
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_pro_color_toload)) { ?>
					<?php foreach($dataArr_pro_color_toload as $key_loadpro => $value_loadpro): ?>
