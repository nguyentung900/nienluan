	...
	$sql_price2="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and pro_newprice between $gia1 and $gia2";
					}
				}
				$query_price=mysqli_query($conn, $sql_price2);
				$dataArr_price_to_loadpro=array();
				while($row=mysqli_fetch_assoc($query_price)){
					$dataArr_price_to_loadpro[]=$row;
				}
				/*echo '<pre>';
				print_r($dataArr_price_to_loadpro);
				echo '</pre>';*/
					/*echo '<pre>';
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_price_to_loadpro)) { ?>
					<?php foreach($dataArr_price_to_loadpro as $key_loadpro => $value_loadpro): ?>
