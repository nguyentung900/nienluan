	...
	<div class="content">
		<div class="container">
			...
			<?php 
				$gia;
				$khoatrang;
				$flower=trim(substr($khoatrang, 13));
				...
				foreach($dataArr_price as $key_price => $value_price){
					...
					if($gia==$value_price) {
						...
						foreach($dataArr_flower as $key_loadpro => $value_loadpro) {
							if($flower==$value_loadpro){
								 $sql_pro_flower_toload="select * from products pro join categorys cate on  pro.cate_id=cate.cate_id join pictures pic on pro.pro_id=pic.pro_id where pic.pic_type=0 and pro_flower='$flower' and pro_newprice between $gia1 and $gia2";
							}
						}
					}
				}
				$query_pro_flower_toload=mysqli_query($conn, $sql_pro_flower_toload);
				$dataArr_pro_flower_toload=array();
				while($row=mysqli_fetch_assoc($query_pro_flower_toload)){
					$dataArr_pro_flower_toload[]=$row;
				}
				/*echo '<pre>';
			...
			<div class="col-md-8 col-sm-8 women-dresses">
				<div class="women-set1">
					<?php if(!empty($dataArr_pro_flower_toload)) { ?>
					<?php foreach($dataArr_pro_flower_toload as $key_loadpro => $value_loadpro): ?>
